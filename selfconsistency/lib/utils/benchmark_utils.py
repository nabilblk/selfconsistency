...
class EfficientBenchmark():
    ...
    def __init__(self, solver, net_module_obj, net_module_obj_init_params, im,
                 ...
                 dense_compute=False, num_per_dim=30, nb_threads=10, batch_size_feat=64):
        """
        ...
        self.batch_size_feat = batch_size_feat
        ...
        self.label_shape = 1 if not 'num_classes' in net_module_obj_init_params else net_module_obj_init_params[
            'num_classes']

        self.cr = self.update_queue_runner()
        self.auto_close_sess = auto_close_sess
        self.n_responses = n_anchors
    ...
    def update_queue_runner(self):
        # returns a new queue_runner
        # Set indices so that we can initialize CustomRunner

        fn = self.argless
        # TODO: THis may create a problem when feeding multiple images
        ...
        cr = queue_runner.CustomRunner(fn, n_processes=self.num_processes,
                                       n_threads=self.num_threads,patch_size = self.patch_size, 
                                       batch_size = 5 )
        self.original_cr_get_inputs = cr.get_inputs

        def new_cr(batch_size):
            # anc, y, x, bat_a, bat_b
            # anchor inds is the anchors indexes in the the image
            # h_inds,w_inds are the analysed patch indexes
            # im_a is the n_anchor patches
            # im_b is the current patch copied n_anchor times
            self.h_indices_, self.w_indices_, im_b = self.original_cr_get_inputs(
                batch_size)
            # we don't use the label since it's test time
            if self.label_shape == 1:
                return im_b, tf.zeros((batch_size), dtype=tf.int64)
            else:
                return im_b, tf.zeros((batch_size, self.label_shape), dtype=tf.float32)
    ...
    def set_image(self, image):
        ...
        self.max_h_ind = 1 + \
            int(np.floor(
                (self.image.shape[0] - self.patch_size) / float(self.stride)))
        self.max_w_ind = 1 + \
            int(np.floor(
                (self.image.shape[1] - self.patch_size) / float(self.stride)))
        # Generate indexes of all possible patches
        self.indices = np.mgrid[0:self.max_h_ind,
                                0:self.max_w_ind].reshape((2, -1)).T  # (n 2)
        self.n_anchors = int(self.indices.shape[0]/2)
    ...
    def data_fn(self, hind, wind):
        ...
        h_inds = np.array([hind], dtype=np.int64)
        w_inds = np.array([wind], dtype=np.int64)
        # batch_a = self.anchor_patches
        batch_b = util.process_im(np.array([patch], dtype=np.float32))
    ...
    def argless_extract_inds(self,dense = True):
        ...
        if dense:
            ...
            anchors = np.random.choice(self.indices.shape[0],self.n_anchors,replace = False)
            iterator = []
            for anchor in anchors:
                tmp = np.array([self.indices[anchor]]*self.indices.shape[0])
                tmp = np.concatenate([self.indices,tmp],axis = 1)
                iterator.append(tmp)
            iterator = np.concatenate(iterator,axis = 0)

        count = 0
        while count * self.max_bs < len(iterator):
            # each indice is a read into np.mgrid[0:self.max_h_ind, 0:self.max_w_ind]
            # self.max_bs x 4
            yield iterator[count * self.max_bs:(count + 1) * self.max_bs, :]
            count += 1
    ...
    def run_ft(self, num_fts=4096):
        ...
        sum_visited = 0
        # Precompute feature map for every patch
        while True:
            try:
                h_ind_, w_ind_, fts_ = self.solver.sess.run([self.h_indices_,
                                                             self.w_indices_,
                                                             self.solver.net.im_b_feat])
                responses[:, h_ind_, w_ind_[0:h_ind_.shape[0]]] = fts_[
                    0:h_ind_.shape[0]].T
    ...
    def precomputed_analysis_vote_cls(self, num_fts=4096,dense = True):
        def lambda_func(arr, patch_size): 
            res = skimage.util.view_as_windows(arr, (patch_size,patch_size, patch_size, patch_size, 2))
