...
class EfficientBenchmark():
    ...
    def update_queue_runner(self, im):
        ...
        def new_cr(batch_size):
            ...
            self.h_indices_, self.w_indices_, im_b = self.original_cr_get_inputs(batch_size)
    ...
    def data_fn(self, hind, wind):
        ...
        patches = self.image[y_ind + batch_indexes, 
                           x_ind + batch_indexes,
                           :]
    ...
    def argless(self):
        ...
        if self.count >= self.indices.shape[0]:
            raise StopIteration()
        inds = self.indices[self.count:(self.count+self.batch_size_feat)]
        self.count += self.batch_size_feat
